...
(function(window, $, Backbone) {
  ...
  var FlipbookView = SingleView.extend({
    ...
    _turnedEvent: function(event, page) {
      ...
      try {
        var audio = new Audio('js/page-flip.mp3');
        audio.play();
      } catch (e) {}
  ...
  var MiniaturesView = SingleView.extend({
    ...
    _refreshPictureEvent: function(event, pageNumber, $pageElement) {
      var settings = window.FlipbookSettings,
          imgNumber = pageNumber < 10 ? '0' + pageNumber : pageNumber, // Agregar cero inicial si es menor que 10
          imgSrc = settings.pageFolder + imgNumber + '.jpg',
  ...
  var ShareBox = SingleView.extend({
    ...
    initialize: function() {
      ...
      html += '<a title="WhatsApp" class="ui-icon show-hint" id="share-whatsapp"><i class="fab fa-whatsapp"></i> Compartir por WhatsApp</a>';
      html += '</div>';
      this.$el.html(html);
      this.$el.appendTo($("body"));
    },
